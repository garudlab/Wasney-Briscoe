...
depth_file = lz4.frame.open("%ssnps/%s/%s.snps_depth.tsv.lz4" % (parse_midas_data.data_directory, species_name, species_name),"rt", encoding="utf-8") # MW 07/14/2025: decoding bytes like objects in file
info_file = lz4.frame.open("%ssnps/%s/%s.snps_info.tsv.lz4" % (parse_midas_data.data_directory, species_name, species_name),"rt", encoding="utf-8") # MW 07/14/2025: decoding bytes like objects in file
...
while True:
    ...
    variant_type = info_items[15] # MW 07/14/2025: Changed this from 5 to 15 due to new column structure of the info dataframe
        
    # make sure it is either a 1D or 4D site
    if not variant_type in allowed_variant_types:
        continue
    
    gene_name = info_items[14] # MW 07/14/2025: Changed this from 6 to 14 due to new column structure of the info dataframe
        
    items = depth_line.split()   
    depths = numpy.array([int(item) for item in items[1:]])
...
output_file = bz2.open("%ssnps/%s/coverage_distribution.txt.bz2" % (parse_midas_data.data_directory, species_name),"wt", encoding = "utf-8") # MW 07/09/2025: encoding as bytes like object
...
output_file = bz2.open("%ssnps/%s/full_coverage_distribution.txt.bz2" % (parse_midas_data.data_directory, species_name),"wt", encoding = "utf-8") # MW 07/09/2025: encoding as bytes like object
output_file.write("SampleID\tD,n(D) ...")
for sample in samples:
    output_file.write("\n")
...
output_file = bz2.open("%ssnps/%s/gene_coverage.txt.bz2" % (parse_midas_data.data_directory, species_name),"wt", encoding = "utf-8") # MW 07/09/2025: encoding as bytes like object
