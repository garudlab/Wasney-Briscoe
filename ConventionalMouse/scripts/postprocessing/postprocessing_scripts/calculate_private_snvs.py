...
min_sample_size = 4                                        #MW: changed this from 6 to 4 (4 samples means at least 1 is from a different subject)
...
if __name__=='__main__':
    ...
    for species_name in good_species_list:
        ...
        print("snp_samples: %s" % (snp_samples))
        ...
        sys.stderr.write("private snvs found: %d \n" % len(chunk_private_snvs))
        if len(private_snvs)>0:
            
            intermediate_filename = intermediate_filename_template % (private_snv_directory, species_name)
            
            # Now add records
            sys.stderr.write("Writing output file to %s\n" % intermediate_filename)
            output_file = gzip.open(intermediate_filename,"wt", encoding="utf-8") # MW 07/15/2025: decoding bytes like object
            # Header
