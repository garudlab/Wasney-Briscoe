...
def parse_snp_prevalences(desired_species_name):
    ...
    file = gzip.open(intermediate_filename,"rt", encoding="utf-8") # MW 07/10/2025: decoding bytes like object
    file.readline()
    for line in file:
        items = line.split(",")
        contig = items[0]
...
if __name__=='__main__': 
    ...
    #sys.stderr.write('Making the directory %ssnp_prevalences \n' % config.data_directory)

    # Open post-processed MIDAS output
    snp_file =  bz2.open("%ssnps/%s/annotated_snps.txt.bz2" % (config.data_directory, species_name),"rt", encoding="utf-8") # MW 07/10/2025: decoding bytes like object
    ...
    for line in snp_file:
        ...
        if num_sites_processed > chunk_size: #MW 1/8/24: stops the script if you just load a small chunk
            break
    ...
    #sys.stderr.write('Intermediate file path: %s \n' % intermediate_filename)
    output_file = gzip.open(intermediate_filename, mode = "wt", encoding = "utf-8") # MW 07/10/2025: encoding as bytes like object
    output_file.write("\n".join(record_strs))
    output_file.close()
