...
substitution_rate_directory = '%ssubstitution_rates/' % (parse_midas_data.data_directory)
intermediate_filename_template = '%s%s.txt.gz'  

min_coverage = config.min_median_coverage
min_sample_size = 2 #changes this from 10 to 2
...
# This definition is called whenever another script downstream uses the output of this data.
    ...
    file = gzip.open(intermediate_filename,"rt", encoding="utf-8") # MW 07/15/2025: decoding bytes like object
...
def calculate_mutrev_matrices_from_substitution_rate_map(substitution_rate_map, type, allowed_samples=[]): 
    ...
    sample_idx_map = {allowed_samples[i]:i for i in range(0,len(allowed_samples))}
...
if __name__=='__main__':
    ...
    parser.add_argument("--use_HMP", help="Use HMP-polarized within-person SFSs when calculating haploid samples", action = "store_true")
    ...
    for species_name in good_species_list:
        ...
        for idxs in [same_subject_idxs, diff_subject_idxs]:
            for sample_pair_idx in range(0,len(idxs[0])):
        ...
        intermediate_filename = intermediate_filename_template % (substitution_rate_directory, species_name)
        file = gzip.open(intermediate_filename,"wt", encoding = "utf-8") # MW 07/15/2025: encoding as bytes like object
        record_str = "\n".join(record_strs)
