import tarfile, bz2, pickle, os, sys, lz4.frame
   ...
   # import cPickle
    ...
    for gene_name in allele_counts_map.keys():
        ...
        for var_type in allele_counts_map[gene_name].keys():
            ...
            for snp_idx in range(0,len(locations)):
                ...
                four_base_counts = [np.hstack([allele_counts[snp_idx,sample_idx,:]*good_idxs[sample_idx], [0,0]]) for sample_idx in range(0,depths.shape[1])]   
    ...
    pickle.dump(snp_alignment, open(snp_file, 'wb'))
    pickle.dump(snp_locations, open(location_file, 'wb'))
    pickle.dump(snp_samples, open(sample_file, 'wb'))
