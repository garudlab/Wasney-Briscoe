...
def calculate_sample_freqs(allele_counts_map, passed_sites_map, variant_type='4D', allowed_genes=None, fold=True):
    ...
    sample_freqs = [[] for i in range(0,allele_counts_map[allele_counts_map.keys()[0]][variant_type]['alleles'].shape[1])]
...
def calculate_pooled_freqs(allele_counts_map, passed_sites_map, allowed_sample_idxs=[], allowed_variant_types = set(['1D','2D','3D','4D']), allowed_genes=set([]), lower_threshold=0.2,upper_threshold=0.8):
    ...
    if len(allowed_sample_idxs)==0:
        ...
        allowed_sample_idxs = numpy.array([True for i in range(0,allele_counts_map.values()[0].values()[0]['alleles'].shape[1])])
...
upper_threshold=config.consensus_upper_threshold, min_change=config.fixation_min_change, min_depth = 1, all_changes = False): #MW 12/6/23: added all_changes flag
    ...
    mut_fixation_matrix = numpy.zeros_like( list(list(passed_sites_map.values())[0].values())[0]['sites'] )*1.0   # MW 07/11/2025: making dictionary keys lists due to dictionary keys not being subscriptable in python 3.X
    ...
    for gene_name in allowed_genes:
        ...
        for variant_type in passed_sites_map[gene_name].keys():
            ...
            if all_changes: #MW 12/6/23: if-else statement to facilitate all_changes flag
                derived_sites = (freqs>=0.5)
                ancestral_sites = (freqs<0.5)
            else:
                derived_sites = (freqs>=upper_threshold)
                ancestral_sites = (freqs<=lower_threshold)
            ...
            # site*sample*sample matrix of sites with sufficient coverage in both samples #
            passed_depths = (depths>0)[:,:,None]*(depths>(min_depth - 1))[:,None,:] #MW: used to be depths>0, but reated argument to set min_depth
...
upper_threshold=config.consensus_upper_threshold, min_change=config.fixation_min_change, min_depth = 1, all_changes = False): #MW 12/5/23: add all_changes flag
    
    if all_changes: #MW 12/5/23: add this if-else statement to facilitate the all_changes flag
    ...
    upper_threshold, min_change, min_depth = min_depth, all_changes = True)
    else:
        mut_fixation_matrix, rev_fixation_matrix, mut_opportunity_matrix, rev_opportunity_matrix = calculate_mutation_reversion_matrix(allele_counts_map, passed_sites_map, allowed_variant_types, allowed_genes, lower_threshold, 
    upper_threshold, min_change, min_depth = min_depth)        
...
upper_threshold=config.consensus_upper_threshold, log10_depth_ratio_threshold=config.fixation_log10_depth_ratio_threshold, min_depth = 1, all_changes = False):
    ...
    for gene_name in allowed_genes:
        ...
        for variant_type in allele_counts_map[gene_name].keys():
            ...
            passed_depths = (depths>(min_depth - 1))[:,0]*(depths>(min_depth - 1))[:,1]*(log10_depth_ratios<log10_depth_ratio_threshold) #MW: changed depths>0 to depths>(min_depth - 1), as this allows me to control the minimum depth of an observed site
            

            if all_changes: #MW 11/29/23: added to collect any kind of polymoprhism
                #minimum_read_support = abs(allele_counts[:,0,0] - allele_counts[:,1,0]) > 1 #Basically, you need more than one read supporting a change
                mutations = (alt_freqs[:,0]<alt_freqs[:,1])*passed_depths
                reversions = (alt_freqs[:,0]>alt_freqs[:,1])*passed_depths
            else:
...
def calculate_haploid_samples(species_name, min_coverage=config.min_median_coverage, threshold_pi=config.threshold_pi, threshold_within_between_fraction=config.threshold_within_between_fraction,debug=False, quick_and_dirty=False, quick_and_dirty_threshold = 5e-4):
    ...
    for sample in desired_samples:
        ...
        if quick_and_dirty:
            ...
            if float(within_sites)/total_sites < quick_and_dirty_threshold: # Original 1e-3
    ...
    for D1,pD1 in zip(D1s,pD1s):
        for D2,pD2 in zip(D2s,pD2s):
            for f,pf in zip(fs,pfs):
                for df in dfs:
                    perrs[df] += 2*binom.cdf(int(D1*(1-df)/2), int(D1), f)*binom.cdf(int(D2*(1-df)/2), int(D2), 1-f)*pD1*pD2*pf #MW: coercing n and k into integers to eliminate a warning I was getting
