...
def calculate_sample_idx_map(sample_list_from, sample_list_to):
    ...
    sample_map = {}
    for i in range(0,len(sample_list_from)):
...
#     new_sample_list = []                             #MW: I commented this out because it is specifically for HMP data.
#     for sample in sample_list:
#         if sample.endswith('c'):
...
def calculate_ordered_subject_triplets(sample_order_map, sample_list=[]):
    ...
    for i in range(0,len(sample_list)):
        ...
        for j in range(0,len(sample_list)):
            ...
            for k in range(0,len(sample_list)):
...
def calculate_sample_subject_matrix(samples):
    ...
    sample_idx_map = {samples[i]:i for i in range(0,len(samples))}

    subject_sample_map = parse_subject_sample_map() #MW made this parse_midas_data.parse_subject_sample_map() instead of parse_subject_sample_map() #MW: changed back because it was stemming from a config issue; It is now being imported from parse_midas_data from the config
    subjects = list(subject_sample_map.keys()) #MW 07/15/2025: made this indexable
    
    sample_subject_matrix = numpy.zeros((len(samples),len(subjects)),dtype=numpy.bool_)
...
###############################################################################
# For a given list of samples, outputs one nested arrays and on dictionary (a map).
#
# Array: This has two nested array containing all possible sample combinations. 
# The upper array has sample 1 of every pair, and the lower array has sample 2 of
# every pair.
#
# Map: The key is the index of each pair in the aforementioned array. The key
# contains a list with two elements: the first is the location of sample 1 of the 
# relevant pair in the original snp input array, while the second is the location 
# of sample 1 of the relevant pair in the original snp input array.
#
###############################################################################

def load_all_sample_pairs(snp_samples = []):
    
    if len(snp_samples) == 0:
        snp_samples = diversity_utils.calculate_highcoverage_samples(species_name, min_coverage)
        

    #make combined_sample_array
    sample_pairs_upper = []
    sample_pairs_lower = []
    counter = 0
    for first_sample in sorted(snp_samples):
        if first_sample not in sample_pairs_upper:
            counter += 1
            for second_sample in sorted(snp_samples)[counter:]:
                sample_pairs_upper.append(first_sample)
                sample_pairs_lower.append(second_sample)
        else:
            raise ValueError("A sample seems to be repeated in the input array.")

    upper_array = numpy.array(sample_pairs_upper, dtype=str)
    lower_array = numpy.array(sample_pairs_lower, dtype=str)
    combined_sample_array = numpy.vstack((upper_array, lower_array))

    #making the sample_pairs_to_input_array_map dictionary
    sample_pairs_to_input_array_map = {}

    for pair_idx in range(0,len(combined_sample_array[0])):

        sample_i = combined_sample_array[0,pair_idx]
        sample_j = combined_sample_array[1,pair_idx]

        i = numpy.where(snp_samples == sample_i)[0][0]
        j = numpy.where(snp_samples == sample_j)[0][0]

        if pair_idx not in sample_pairs_to_input_array_map:
            sample_pairs_to_input_array_map[pair_idx] = []

        sample_pairs_to_input_array_map[pair_idx].append(i)
        sample_pairs_to_input_array_map[pair_idx].append(j)
        
    return combined_sample_array, sample_pairs_to_input_array_map
