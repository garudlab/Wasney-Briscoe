...
def calculate_haploid_samples(species_name, min_coverage=config.min_median_coverage, threshold_pi=config.threshold_pi, threshold_within_between_fraction=config.threshold_within_between_fraction,debug=False, quick_and_dirty=False):
    ...
    for sample in desired_samples:
        ...
        if quick_and_dirty:
            ...
            if float(within_sites)/total_sites < 1e-3:
