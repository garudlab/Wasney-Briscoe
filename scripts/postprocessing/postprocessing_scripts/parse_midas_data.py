...
def parse_sample_metadata_map(): 
    ...
    file = open(config.analysis_directory+"metadata/metadata.csv","r")
    file.readline() # header
    for line in file:
        items = line.split("\t")
        sample_id = items[0].strip()
        accession_id = items[1].strip()
        tissue_type = items[2].strip()
        subject_id = items[3].strip()
        diet = items[4].strip()
        location = items[5].strip()
        
        sample_metadata_map[sample_id] = (accession_id, tissue_type, subject_id, diet, location)
...
def parse_subject_sample_map(sample_metadata_map = {}): 
    ...
    for sample_id in sample_metadata_map:
        accession_id, tissue_type, subject_id, diet, location = sample_metadata_map[sample_id]
...
def parse_sample_order_map(sample_metadata_map = {}): 
    ...
    for sample_id in sample_metadata_map:
        accession_id, tissue_type, subject_id, diet, location = sample_metadata_map[sample_id]
        if tissue_type == "Duodenum":
            order = 1
        elif tissue_type == "Jejunum":
            order = 2
        elif tissue_type == "Ileum":
            order = 3
        elif tissue_type == "Cecum":
            order = 4
        elif tissue_type == "Colon":
            order = 5
        elif tissue_type == "Stool":
            order = 1
        else:
...
def parse_polarized_snps(species_name, debug=False, allowed_samples=[], allowed_genes=[], allowed_variant_types=['1D','2D','3D','4D'], initial_line_number=0, chunk_size=1000000000,snps_directory=default_snps_directory): #MW: removed host argument
   ...
   #     allowed_sample_set = (set(allowed_samples) & set(samples))
    ...
    sample_order = pd.read_csv("%ssnps/%s/snps_summary.txt" % (data_directory, species_name),sep="\t",index_col=0).index
    sample_order = numpy.array(list(sample_order))
    print(sample_order)
    # This is a hack because there were some mistaken repeats in an old data file
    # should be able to remove later
    #seen_samples = set()
