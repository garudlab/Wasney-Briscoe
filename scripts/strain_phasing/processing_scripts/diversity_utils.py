...
#
def calculate_consensus_genotypes(allele_counts_matrix,lower_threshold=0.2,upper_threshold=0.8,prevalence_thresh=0.0):
...
#####################################################################
def calculate_D_prime(allele_counts_1, allele_counts_2):
    # Another standard measure of linkage disequilibrium:
    #
    # D' = D/D_max
    # D = p_ab - p_a*p_b
    # D_max = max(-p_a*p_b, -(1-p_a)*(1-p_b)) if D < 0
    # D_max = max(p_a*(1-p_b), (1-p_a)*p_b)) if D > 0
    ...
    # D (p_ab-p_a*p_b)
    D = joint_pooled_freqs-marginal_pooled_freqs_1*marginal_pooled_freqs_2
    
    if D < 0:
        D_max = numpy.maximum(-marginal_pooled_freqs_1*marginal_pooled_freqs_2, -(1 - marginal_pooled_freqs_1)*(1 - marginal_pooled_freqs_2))
    elif D > 0:
        D_max = numpy.maximum(marginal_pooled_freqs_1*(1-marginal_pooled_freqs_2), (1 - marginal_pooled_freqs_1)*marginal_pooled_freqs_2)        
    else:
        D_max = 1
...
##################################
def generate_haplotype(allele_counts_4D, allele_counts_1D, location_dictionary, species_name,gene_name):
    ...
    locations=list(location_dictionary.keys())
    locations=sorted(locations)
   
    #s_consensus='' # store the haplotypes in a string for printing out later
    #s_annotation=''
    outFile_consensus=open(os.path.expanduser('/u/scratch/r/rwolff/LD/tmp_intermediate_files/%s/tmp_consensus_%s.txt') % (species_name,gene_name) ,'w')
    outFile_anno=open(os.path.expanduser('/u/scratch/r/rwolff/LD/tmp_intermediate_files/%s/tmp_anno_%s.txt') % (species_name,gene_name) ,'w')
        ...
        for person in range(0, len(alleles)):
            ...
            if coverage[person] == 0.0:
                ...
                if annotation[person] ==0:
                    ...
                    if variant_type=='4D':
                        annotation[person] = str(4) # fixed syn diff from ref
                    else:
                        #annotation[person]=str(2) # fixed nonsyn diff from ref
                        annotation[person] = str(1)
                else: 
                    if variant_type=='4D':
                        #annotation[person]=str(3) # polymorphic syn within host
                        annotation[person] = str(4)
                    else:
                        annotation[person] = str(1) # polymorphic nonsyn within host
...
def calculate_sample_freqs(allele_counts_map, passed_sites_map, variant_type='4D', allowed_genes=None, fold=True):
    ...
    sample_freqs = [[] for i in range(0,allele_counts_map[list(allele_counts_map.keys())[0]][variant_type]['alleles'].shape[1])]
    
    passed_sites = numpy.zeros(passed_sites_map[list(passed_sites_map.keys())[0]][variant_type]['sites'].shape[0])*1.0
    ...
    for gene_name in allowed_genes:
        ...
        for sample_idx in range(0,freqs.shape[1]):
...
def calculate_temporal_sample_freqs(allele_counts_map, passed_sites_map, initial_sample_idx, final_sample_idx, allowed_variant_types=set(['1D','2D','3D','4D']), allowed_genes=None):
    ...
    for gene_name in allowed_genes:
        for variant_type in list(allele_counts_map[gene_name].keys()):
    ...
    for gene_name in allowed_genes:
        for variant_type in list(allele_counts_map[gene_name].keys()):
            ...
            if changed_sites.sum() > 0:
                ...
                gene_names.extend( int(changed_sites.sum()) * [gene_name] )
...
def calculate_sample_freqs_2D(allele_counts_map, passed_sites_map, desired_samples, variant_type='4D', allowed_genes=None, fold=True):
    ...
    sample_freqs = [[] for i in range(0, num_samples)]
    joint_passed_sites= [[] for i in range(0, num_samples)]
...
def calculate_pooled_freqs(allele_counts_map, passed_sites_map, allowed_sample_idxs=[], allowed_variant_types = set(['1D','2D','3D','4D']), allowed_genes=set([]), lower_threshold=0.2,upper_threshold=0.8):
    ...
    if len(allowed_sample_idxs)==0:
        ...
        allowed_sample_idxs = numpy.array([True for i in range(0,list(allele_counts_map.values())[0].values()[0]['alleles'].shape[1])])
...
upper_threshold=config.consensus_upper_threshold):
    ...
    for gene_name in allowed_genes:
        ...
        for variant_type in list(passed_sites_map[gene_name].keys()):
...
upper_threshold=config.consensus_upper_threshold):
    ...
    doubleton_matrix = numpy.zeros_like( list(passed_sites_map.values())[0].values()[0]['sites'] )*1.0   
...
upper_threshold=config.consensus_upper_threshold, min_change=config.fixation_min_change):
    ...
    mut_fixation_matrix = numpy.zeros_like( list(passed_sites_map.values())[0].values()[0]['sites'] )*1.0   
...
upper_threshold=config.consensus_upper_threshold, min_change=config.fixation_min_change):
    ...
    fixation_matrix_mutation = numpy.zeros_like(list(passed_sites_map.values())[0].values()[0]['sites'])*1.0 
...
def calculate_new_snp_matrix(allele_counts_map, passed_sites_map, allowed_variant_types=set([]), allowed_genes=set([]), min_freq=0.05, max_freq=0.2):
    ...
    new_snp_matrix = numpy.zeros_like(list(passed_sites_map.values())[0].values()[0]['sites'])*1.0  
...
def calculate_pi_matrix(allele_counts_map, passed_sites_map, variant_type='4D', allowed_genes=None):
    ...
    pi_matrix = numpy.zeros_like(passed_sites_map[list(passed_sites_map.keys())[0]][variant_type]['sites'])*1.0
    avg_pi_matrix = numpy.zeros_like(pi_matrix)
    passed_sites = numpy.zeros_like(pi_matrix)
    
    for gene_name in allowed_genes:
        
        if gene_name in list(passed_sites_map.keys()):
...
upper_threshold=config.consensus_upper_threshold, log10_depth_ratio_threshold=config.fixation_log10_depth_ratio_threshold):
    ...
    for gene_name in allowed_genes:
        ...
        if gene_name not in list(allele_counts_map.keys()):
            continue
...
def calculate_mean_pi_matrix_per_pathway(pi_per_gene, avg_pi_per_gene, passed_sites_per_gene,num_people_with_data, kegg_ids,min_passed_sites_per_person=100):
    ...
    gene_name=list(avg_pi_per_gene.keys())[0]
    ...
    for gene_name in list(avg_pi_per_gene.keys()):
        pathway=kegg_ids[gene_name][0][1]
        if pathway not in list(avg_pi_per_pathway.keys()):
    ...
    for pathway_name in list(avg_pi_per_pathway.keys()):
...
def calculate_mean_fixation_matrix_per_pathway(fixation_per_gene, passed_sites_per_gene,num_people_with_data, kegg_ids, min_passed_sites_per_person=100):
    ...
    gene_name=list(fixation_per_gene.keys())[0]
    ...
    for gene_name in list(fixation_per_gene.keys()):
        pathway=kegg_ids[gene_name][0][1]
        if pathway not in list(fixation_per_pathway.keys()):
    ...
    for pathway_name in list(fixation_per_pathway.keys()):
       # we want to identify people that have few passed sites even after aggregating the data accross genes. Then set the values in these cells to zero because these data points are too noisy
        low_passed_sites_idxs=passed_sites_per_pathway[pathway_name]<min_passed_sites_per_person
        passed_sites_per_pathway[pathway_name][low_passed_sites_idxs]=0
...
def calculate_highcoverage_samples(species_name, min_coverage=config.min_median_coverage):
    ...
    median_coverages = numpy.array([sample_coverage_map[samples[i]] for i in range(0,len(samples))])
...
def calculate_triple_temporal_samples(species_name, min_coverage=config.min_median_coverage):
    ...
    temporal_samples = set()
    for sample_triplet_idx in range(0,len(same_subject_idxs)):
