...
def parse_global_marker_gene_coverages():
    ...
    file.close()    
    species, species_coverage_matrix = list(zip(*sorted(zip(species, species_coverage_matrix), key=lambda pair: pair[1].sum(), reverse=True)))
...
def parse_snps(species_name, debug=False, allowed_samples=[], allowed_genes=[], allowed_variant_types=['1D','2D','3D','4D'], initial_line_number=0, chunk_size=1000000000):
    ...
    for gene_name in list(passed_sites_map.keys()):
...
def parse_within_sample_sfs(species_name, allowed_variant_types=set(['1D','2D','3D','4D'])):
    ...
    sfs_file = bz2.BZ2File("%ssnps/%s/within_sample_sfs.txt.bz2" % (data_directory, species_name),"rb")
...
def load_gene_centroid_map(desired_species_name):
    ...
    for gene_id in list(gene_centroid_map.keys()):
...
def parse_subject_sample_time_map(filename=os.path.expanduser("~/ben_nandita_hmp_scripts/HMP1-2_ids_order.txt")): 
    ...
    for line in file:
        ...
        if visno not in list(subject_sample_time_map[subject_id].keys()):
...
def collapse_visno_reps_subject_sample_time_map(subject_sample_time_map):
    ...
    for subject in subject_sample_time_map:
        ...
        for visno in list(subject_sample_time_map[subject].keys()): # loop over samples
...
def prune_subject_sample_time_map(subject_sample_time_map_all_samples,sample_coverage_map):
    ...
    for subject in list(subject_sample_time_map_all_samples.keys()): # loop over subjects (hosts)
        for visno in list(subject_sample_time_map_all_samples[subject].keys()): # loop over samples
            keep_coverage=0
            keep_sample=''
            keep_day=0
            if len(subject_sample_time_map_all_samples[subject][visno]) >1: # find the sample with highest cov
                ...
                for i in range(0,len(subject_sample_time_map_all_samples[subject][visno])): 
                    ...
                    if sample in list(sample_coverage_map.keys()):
            ...
            if keep_sample !='':
                if subject not in list(subject_sample_time_map.keys()):
...
def calculate_time_pairs(subject_sample_time_map, samples):
    ...
    for subject_id in list(subject_sample_time_map.keys()):
        visnos=list(subject_sample_time_map[subject_id].keys()) #visit numbers
    ...
    for subject_id in list(subject_sample_time_map.keys()):
        ...
        if (len(visnos) > 1) and (1 in visnos):           
            if (subject_sample_time_map[subject_id][1][0][0] in samples): #check if first visit in samples 
                ...
                for i in [2,3]:
                    if i in list(subject_sample_time_map[subject_id].keys()) and subject_sample_time_map[subject_id][i][0][0] in samples and unique_pair_found==False:
                        index1.append(samples.tolist().index(subject_sample_time_map[subject_id][1][0][0])) 
                        index2.append(samples.tolist().index(subject_sample_time_map[subject_id][i][0][0]))
                        visno.append(i)
                        day.append(subject_sample_time_map[subject_id][i][0][1])
